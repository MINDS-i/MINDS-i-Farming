...
void loop() {
    ...
    if (numPingSensors > 0 && millis() >= (lastPingUpdate + PING_UPDATE_PERIOD_MS)) {
        ...
        pingIter =
            (pingIter + 1) % numPingSensors;    // since pingIter starts at 0, this will start at the second ping sensor
...
void echoCheck() { // Timer2 interrupt calls this function every 24uS where you can check the ping status.
    if (sonars[pingIter].check_timer()) { // This is how you check to see if the ping was received.
